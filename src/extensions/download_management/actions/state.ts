...
export const pauseDownload = safeCreateAction(
  'PAUSE_DOWNLOAD',
  (id: string, paused: boolean, chunks: IChunk[]) => (
    { id, paused, chunks }
  )
);
...
export const setDownloadInstalled = safeCreateAction(
  ...
  (id: string, gameId: string, modId: string) => (
    { id, gameId, modId }
  )
);
