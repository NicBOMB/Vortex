...
export const setHideTopLevelCategory = safeCreateAction(
  'SET_HIDE_TOPLEVEL_CATEGORY',
  (hide: boolean) => ({ hide })
);

export const showUsageInstruction = safeCreateAction(
  'SHOW_USAGE_INSTRUCTION',
  (usageId: string, show: boolean) => ({ usageId, show })
);

export const setRelativeTimes = safeCreateAction(
  'SET_RELATIVE_TIMES',
  (enabled: boolean) => enabled
);
