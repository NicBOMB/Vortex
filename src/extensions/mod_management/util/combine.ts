...
async function combineMods(api: IExtensionApi, gameId: string, modIds: string[]) {
  ...
  try {
    ...
    await fs.renameAsync(path.join(stagingPath, target.installationPath),
                         path.join(stagingPath, tempName + '_delme'));
    try {
      await fs.renameAsync(path.join(stagingPath, tempName),
      ...
      await fs.renameAsync(path.join(stagingPath, tempName + '_delme'),
                           path.join(stagingPath, target.installationPath));
    }
